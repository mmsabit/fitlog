...
const MyPlan = () => {
  ...
  const { addWorkOut, saveWorkOut, isactive, setIsactive, } = useContext(WorkOutContext) as {
      addWorkOut: fitType[];
      saveWorkOut: fitType[];
      isactive: isactive;
  ...
  return (
    <div className="my-10 container mx-auto max-w-9/10">
      ...
      </div>{isactive === "plan" ? <Myplanpoint workouts={addWorkOut} /> : <Myplanpoint workouts={saveWorkOut} />}
      ...
      <div className="flex lg:flex-row flex-col gap-6 justify-between items-center">
        <div className="bg-[#151921] p-2 border border-[#232732] rounded-xl ">
          <button className={`${isactive === "plan" ? "text-white bg-[#1F242D] border-[#2B303D] font-bold" : "text-[#8A92A0] bg-transparent border-transparent"} text-sm  rounded-lg  py-1.5 px-4 btn shadow-none`}
          onClick={() => {handleisactive("plan")}}
          >
            Today&apos;s Plan
          </button>
          <button className={`${isactive === "saved" ? "text-white bg-[#1F242D] border-[#2B303D] font-bold" : "text-[#8A92A0] bg-transparent border-transparent"} text-sm  rounded-lg  py-1.5 px-4 btn shadow-none`}
          onClick={() => {handleisactive("saved")}}
          >
            Saved
          </button>
        </div>
      ...
      <div className="mt-5">
        {isactive === "plan" ? <MyPlanMid workouts={addWorkOut} /> : <MyPlanMid workouts={saveWorkOut} />}
      </div>
    </div>
  );
};
